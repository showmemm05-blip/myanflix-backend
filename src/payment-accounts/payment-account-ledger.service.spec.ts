...
/** The staff member recording manual entries — `id` is what performedByUserId checks. */
const admin = {
  id: 'admin-1',
  username: 'boss',
...
describe('PaymentAccountLedgerService', () => {
  ...
  describe('recordManualEntry (via applyMovement)', () => {
    ...
    it('credits: increments balance and totalIn, derives balanceBefore/After correctly', async () => {
      ...
      prisma.paymentAccount.update.mockResolvedValue(
        makeAccount({
          ...
          totalIn: new Prisma.Decimal(1500),
        }),
      );
      prisma.paymentAccountTransaction.create.mockImplementation(
        ({ data }) => data,
      );
      ...
      const { entry } = await service.recordManualEntry(
        'acct-1',
        {
          type: PaymentAccountTransactionType.MANUAL_CREDIT,
          amount: 500,
          note: 'top-up',
        },
        admin,
    ...
    it('debits: increments totalOut, never guards against going negative (internal ledger, not a user-facing spend limit)', async () => {
      prisma.paymentAccount.findUnique.mockResolvedValue(
        makeAccount({ balance: new Prisma.Decimal(100) }),
      );
      prisma.paymentAccount.update.mockResolvedValue(
        makeAccount({
          balance: new Prisma.Decimal(-400),
          totalOut: new Prisma.Decimal(500),
        }),
      );
      prisma.paymentAccountTransaction.create.mockImplementation(
        ({ data }) => data,
      ...
      expect((call.data.balance.increment as Prisma.Decimal).toNumber()).toBe(
        -500,
      );
    });
  });
  ...
  describe('syncDepositLink', () => {
    ...
    it('links a fresh account: claims the FK and posts a DEPOSIT_IN entry, no reversal (nothing to reverse)', async () => {
      ...
      prisma.paymentAccount.update.mockResolvedValue(
        makeAccount({ balance: new Prisma.Decimal(3000) }),
      );
      prisma.paymentAccountTransaction.create.mockImplementation(
        ({ data }) => data,
      );
      ...
      const created =
        prisma.paymentAccountTransaction.create.mock.calls[0][0].data;
    ...
    it('re-linking: reverses the old account (ADJUSTMENT_DEBIT) then posts DEPOSIT_IN on the new one', async () => {
      ...
      expect(prisma.paymentAccount.update.mock.calls[0][0].where).toEqual({
        id: 'acct-old',
      });
      expect(prisma.paymentAccount.update.mock.calls[1][0].where).toEqual({
        id: 'acct-new',
      });
      const [reversalCall, forwardCall] =
        prisma.paymentAccountTransaction.create.mock.calls;
      expect(reversalCall[0].data.type).toBe(
        PaymentAccountTransactionType.ADJUSTMENT_DEBIT,
      );
      expect(forwardCall[0].data.type).toBe(
    ...
    it('clearing the link (new id null): only reverses, posts no new entry', async () => {
      ...
      expect(prisma.paymentAccountTransaction.create).toHaveBeenCalledTimes(1);
      expect(
        prisma.paymentAccountTransaction.create.mock.calls[0][0].data.type,
      ).toBe(PaymentAccountTransactionType.ADJUSTMENT_DEBIT);
    });
  ...
  describe('syncWithdrawalLink', () => {
    ...
    it('no-ops when the payment account is unchanged', async () => {
      await service.syncWithdrawalLink(
        prisma as never,
        withdrawal,
        null,
        null,
    ...
    it("re-linking: reverses the old account with ADJUSTMENT_CREDIT (opposite direction from deposits' ADJUSTMENT_DEBIT)", async () => {
      ...
      expect(forwardCall[0].data.type).toBe(
        PaymentAccountTransactionType.WITHDRAWAL_OUT,
      );
    });
